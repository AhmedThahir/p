...
var RevealCountDown =
  ...
  (function() {
    ...
    if(options.tickSound)
			autostart = options.autostart
		if(options.ending_color)
			ending_color = options.ending_color
		var endSound =
      ...
      interval = setInterval(async function() {
        if (elapsedTime < startTime && running && !Reveal.isPaused()) {
          ...
          if (tick && startTime < elapsedTime + options.playTickSoundLast)
					{
						counterRef.style.color = ending_color
            tick.play();
          }
    ...
    function initCountDown(currentSlide) {
      ...
      if (counterRef == null) return;
			...
			autostart_custom = counterRef.getAttribute("autostart");
			if (autostart_custom)
			{
